...
<div class="m-4">
    <h1 class="text-4xl text-center font-['Fredoka_One']">Ma Todo List</h1>

    <div class="text-center my-2">
        <button class="border border-black px-4 py-2 rounded-xl font-bold bg-yellow-200 relative group hover:rounded-3xl transition-all hover:bg-yellow-300"
                on:click={createTaskModal}>
            <span class="block top-2 left-1 absolute px-4 py-2 rounded-xl font-bold bg-yellow-200 -z-10 bg-black w-full h-full group-hover:rounded-3xl transition-all"></span>
            Créer une tâche
        </button>
    </div>
...
<Modal visible="{showCreateTaskModal}" title="Nouvel évènement" on:closeModal={closeCreateTaskModal}>
